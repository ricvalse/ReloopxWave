...
- ADRs in `docs/decisions/` (0001 monorepo, 0002 pgvector, 0003 consolidated worker, 0004 all-Railway deploy, 0005 360dialog channel creation, 0006 whatsapp templates + flows, 0007 GHL marketplace agency-install, 0008 persona strutturata + consegna "umana", 0009 playground = preview fedele del flusso WhatsApp reale, 0010 playground dry-run: simulazione tool + stato lead evolutivo + consegna realistica, 0011 automazioni: timing dal grafo, 0014 **automazioni: tutto dalla lavagnetta, niente hardcoded** (contenuto SOLO da `step.free_text`/template del nodo canvas — rimossi i testi hardcoded degli scheduler; `decide_outbound` senza `fallback_text`: nodo `send` vuoto → skip `no_content`, `step is None` → skip `no_flow`; seeding `enabled=False`; inverte "attivi di default"+fallback di 0011)). Make a non-obvious call during implementation → write an ADR, don't bury it in a commit.
