...
3. 🔴 **Scope** (da `oauth.py:DEFAULT_SCOPES` — TUTTI e 15, la lista completa): `contacts.readonly`, `contacts.write`, `opportunities.readonly`, `opportunities.write`, `calendars.readonly`, `calendars.write`, `calendars/events.readonly`, `calendars/events.write`, `locations.readonly`, `oauth.readonly`, `oauth.write`, `conversations.readonly`, `conversations.write`, `conversations/message.readonly`, `conversations/message.write`. ⚠️ **Gli scope sono BLOCCATI una volta live** (cambiarli = nuova versione dell'app + re-auth dell'agenzia): è questa la lista da azzeccare subito. Senza `oauth.readonly`/`oauth.write` il mint dei token location fallisce con 401 e le location restano `pending_link` per sempre. (I webhook events invece si modificano in qualsiasi momento senza nuova versione.)
