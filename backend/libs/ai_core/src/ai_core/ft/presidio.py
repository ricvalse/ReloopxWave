...
def build_presidio_transform(
    *,
    ...
    so callers can fall back to regex-only redaction without crashing.
    """
    try:
        from presidio_analyzer import AnalyzerEngine
    except Exception as e:  # ImportError or downstream init error
        logger.warning("ft.presidio.unavailable", error=str(e))
        return None
    ...
    try:
        ...
        logger.warning("ft.presidio.init_failed", error=str(e), language=language)
        return None
