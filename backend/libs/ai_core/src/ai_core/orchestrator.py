...
class ConversationOrchestrator:
    ...
    def _build_proactive_messages(
        ...
        system_parts = [
            ctx.system_prompt,
            render_schema_hint(ctx.allowed_actions, tools_available=False, handoff=ctx.handoff),
        ]
        if ctx.scoring_enabled:
            system_parts.append(
                "Stato qualificazione del lead (uso interno, non citarlo al cliente): "
...
def _parse_structured(raw: str) -> _StructuredResponse:
    ...
    Ogni via d'uscita passa dalla normalizzazione dei nomi legacy (ADR 0026): da
    qui in poi esiste solo `handoff_human`, anche sulle azioni recuperate.
    """
    with contextlib.suppress(Exception):
        return _normalize_kinds(_StructuredResponse.model_validate_json(raw))
    ...
    if candidate != raw:
        ...
        with contextlib.suppress(Exception):
            return _normalize_kinds(_StructuredResponse.model_validate_json(candidate))
...
def _normalize_kinds(parsed: _StructuredResponse) -> _StructuredResponse:
    """Traduce i nomi legacy: da qui in poi esiste solo `handoff_human` (ADR 0026)."""
...
def _salvage_actions(raw_actions: Any) -> list[OrchestratorAction]:
    ...
    I nomi legacy vengono normalizzati anche qui: un `escalate_human` recuperato
    da un payload fuori schema deve arrivare al dispatcher col nome corrente,
    altrimenti l'handoff sfuggirebbe al rename dell'ADR 0026.
    """
