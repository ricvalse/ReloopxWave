"""UC-04 — move_pipeline action handler unit tests.

Stubs GHLClient + DB touchpoints. Verifies that:
  1. With an opportunity already stamped on lead.meta, the opportunity is moved.
  2. With NO opportunity yet (lead never booked), one is created directly in the
     target stage — creating it there IS the move (the standalone path).
  3. No GHL integration → reported as failure, no crash.
"""

from __future__ import annotations

import uuid
from contextlib import asynccontextmanager
from dataclasses import dataclass, field
from unittest.mock import AsyncMock, MagicMock

import pytest

from ai_core.actions.pipeline import MovePipelineHandler
from ai_core.conversation_service import TurnContext
from ai_core.orchestrator import OrchestratorAction
from db import ResolvedGHLIntegration


@dataclass
class FakeLead:
    id: uuid.UUID = field(default_factory=uuid.uuid4)
    pipeline_stage_id: str | None = None
    meta: dict | None = None


@pytest.fixture
def turn_ctx() -> TurnContext:
    return TurnContext(
        tenant_id=uuid.uuid4(),
        merchant_id=uuid.uuid4(),
        lead_id=uuid.uuid4(),
        conversation_id=uuid.uuid4(),
        lead_phone="39333000000",
        phone_number_id="PNID-1",
        api_key="test-channel-key",
    )


@pytest.fixture
def ghl_bundle(turn_ctx: TurnContext) -> ResolvedGHLIntegration:
    return ResolvedGHLIntegration(
        merchant_id=turn_ctx.merchant_id,
        tenant_id=turn_ctx.tenant_id,
        access_token="ghl-at",
        refresh_token="ghl-rt",
        expires_at=0,
        location_id="loc-1",
        meta={},
    )


def _patch_session(monkeypatch, *, ghl, lead):
    from ai_core.actions import pipeline as mod

    events: list[dict] = []

    @asynccontextmanager
    async def fake_session(ctx):
        yield object()

    class FakeIntegrationRepo:
        def __init__(self, session, *, kek_base64): ...
        async def resolve_ghl(self, merchant_id):
            return ghl

    class FakeLeadRepo:
        def __init__(self, session): ...
        async def get_by_phone(self, *, merchant_id, phone):
            return lead

    class FakeAnalyticsRepo:
        def __init__(self, session): ...
        async def emit(self, **kw):
            events.append(kw)

    class FakeConfig:
        def __init__(self, session): ...
        async def resolve(self, key, *, merchant_id):
            return {
                "pipeline.qualified_stage_id": "STAGE-QUALIFIED",
                "pipeline.default_pipeline_id": "PIPE-1",
            }.get(getattr(key, "value", str(key)))

    monkeypatch.setattr(mod, "tenant_session", fake_session)
    monkeypatch.setattr(mod, "IntegrationRepository", FakeIntegrationRepo)
    monkeypatch.setattr(mod, "LeadRepository", FakeLeadRepo)
    monkeypatch.setattr(mod, "AnalyticsRepository", FakeAnalyticsRepo)
    monkeypatch.setattr(mod, "ConfigResolver", FakeConfig)
    return events


def _patch_ghl_client(monkeypatch):
    from ai_core.actions import pipeline as mod

    client = AsyncMock()
    client.upsert_contact = AsyncMock(return_value={"contact": {"id": "CT-1"}})
    client.move_opportunity = AsyncMock(return_value={"id": "OPP-1"})
    client.create_opportunity = AsyncMock(return_value={"id": "OPP-NEW"})
    client.close = AsyncMock()
    monkeypatch.setattr(mod, "GHLClient", MagicMock(side_effect=lambda **kw: client))
    return client


async def test_move_existing_opportunity(
    monkeypatch: pytest.MonkeyPatch, turn_ctx: TurnContext, ghl_bundle: ResolvedGHLIntegration
) -> None:
    lead = FakeLead(meta={"ghl_opportunity_id": "OPP-1", "ghl_pipeline_id": "PIPE-1"})
    events = _patch_session(monkeypatch, ghl=ghl_bundle, lead=lead)
    client = _patch_ghl_client(monkeypatch)

    handler = MovePipelineHandler(kek_base64="unused", ghl_client_id="x", ghl_client_secret="y")
    await handler(OrchestratorAction(kind="move_pipeline", payload={}), turn_ctx)

    client.move_opportunity.assert_awaited_once()
    client.create_opportunity.assert_not_awaited()
    assert lead.pipeline_stage_id == "STAGE-QUALIFIED"
    assert events[-1]["event_type"] == "pipeline.moved"


async def test_move_creates_opportunity_when_missing(
    monkeypatch: pytest.MonkeyPatch, turn_ctx: TurnContext, ghl_bundle: ResolvedGHLIntegration
) -> None:
    # Lead never booked: no opportunity stamped on meta. The handler must create
    # one in the target (qualified) stage from the configured default pipeline.
    lead = FakeLead(meta={})
    events = _patch_session(monkeypatch, ghl=ghl_bundle, lead=lead)
    client = _patch_ghl_client(monkeypatch)

    handler = MovePipelineHandler(kek_base64="unused", ghl_client_id="x", ghl_client_secret="y")
    await handler(
        OrchestratorAction(kind="move_pipeline", payload={"contact_fields": {"name": "Mario"}}),
        turn_ctx,
    )

    client.create_opportunity.assert_awaited_once()
    client.move_opportunity.assert_not_awaited()
    assert lead.pipeline_stage_id == "STAGE-QUALIFIED"
    assert lead.meta["ghl_opportunity_id"] == "OPP-NEW"
    assert events[-1]["event_type"] == "pipeline.moved"


async def test_move_no_ghl_integration(
    monkeypatch: pytest.MonkeyPatch, turn_ctx: TurnContext
) -> None:
    lead = FakeLead(meta={})
    events = _patch_session(monkeypatch, ghl=None, lead=lead)
    _patch_ghl_client(monkeypatch)

    handler = MovePipelineHandler(kek_base64="unused", ghl_client_id="x", ghl_client_secret="y")
    await handler(OrchestratorAction(kind="move_pipeline", payload={}), turn_ctx)

    assert events[-1]["event_type"] == "pipeline.failed"
    assert events[-1]["properties"]["reason"] == "no_ghl_integration"
